...
test "perpendicular distance is correct" {
    ...
    const result = seg.project(vec2.xy(5, 5));
...
test "diagonal segment distance" {
    ...
    const px = 5 - offset;
    const py = 5 + offset;
    const result = seg.project(vec2.xy(px, py));
...
test "multiple points have same behavior" {
    ...
    for (positions) |pos| {
        const result = seg.project(vec2.xy(pos.x, 3));
