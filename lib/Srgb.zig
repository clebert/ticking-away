...
fn byteToLinear(byte: u8) f32 {
    return toLinearComponent(@as(f32, @floatFromInt(byte)) / 255.0);
}

/// Inverse sRGB transfer function: a gamma-encoded component in 0–1 to linear light.
pub fn toLinearComponent(normalized: f32) f32 {
...
test "toLinear converts black correctly" {
    ...
    try std.testing.expectEqual(@as(f32, 0.0), linear.vector[0]);
    try std.testing.expectEqual(@as(f32, 0.0), linear.vector[1]);
    try std.testing.expectEqual(@as(f32, 0.0), linear.vector[2]);
    try std.testing.expectEqual(@as(f32, 1.0), linear.vector[3]);
}

test "toLinear converts white correctly" {
    const linear = white.toLinear();

    try std.testing.expectApproxEqAbs(@as(f32, 1.0), linear.vector[0], 1e-6);
    try std.testing.expectApproxEqAbs(@as(f32, 1.0), linear.vector[1], 1e-6);
    try std.testing.expectApproxEqAbs(@as(f32, 1.0), linear.vector[2], 1e-6);
    try std.testing.expectApproxEqAbs(@as(f32, 1.0), linear.vector[3], 1e-6);
}
...
test "toLinear converts mid-gray correctly" {
    ...
    try std.testing.expectApproxEqAbs(@as(f32, 0.5), linear.vector[0], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 0.5), linear.vector[1], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 0.5), linear.vector[2], 0.01);
}
...
test "toLinear preserves alpha as normalized value" {
    ...
    try std.testing.expectApproxEqAbs(@as(f32, 128.0 / 255.0), linear.vector[3], 1e-6);
}

test "byteToLinear uses linear formula below threshold" {
    const result = byteToLinear(10);
...
test "byteToLinear uses gamma formula above threshold" {
    const result = byteToLinear(128);
