...
static inline float fabsf_impl(float x) { return x < 0.0f ? -x : x; }

static inline float maxf_impl(float a, float b) { return a > b ? a : b; }

static inline float minf_impl(float a, float b) { return a < b ? a : b; }

static inline float sqrtf_impl(float x) { return __builtin_sqrtf(x); }
...
static inline float ang_dist(float a, float b) { return fabsf_impl(reduce_angle(a - b)); }
...
static inline float cosf_approx(float x) { return sinf_approx(x + PI / 2.0f); }
...
static inline float fast_log2f(float x) {
  ...
  int32_t e = (int32_t)(v.u >> 23) - 127;
  v.u = (v.u & 0x007FFFFF) | 0x3F800000;
  float m = v.f; // m in [1, 2)

  // Use transformation t = (m-1)/(m+1) which maps [1,2) to [0, 1/3)
  // log2(m) = (2/ln2) * (t + t³/3 + t⁵/5 + ...)
  // This series converges faster and is more accurate than direct polynomial
  float t = (m - 1.0f) / (m + 1.0f);
  float t2 = t * t;

  // Coefficients derived from series: 2/(k*ln(2)) for k = 1, 3, 5
  float log2_m = t * (2.8853900817779268f + t2 * (0.9617966939259756f + t2 * 0.5770780163555854f));
...
static inline float vec2_dot(float ax, float ay, float bx, float by) { return ax * bx + ay * by; }

static inline float vec2_length(float x, float y) { return sqrtf_impl(x * x + y * y); }
